(function () {
  ...
  function initProfilesUi(el, hooks) {
    ...
    function updateBracketDescriptorTitles() {
      ...
      if (el.hintProfileRegular) {
        el.hintProfileRegular.title =
      ...
      if (el.hintProfileGlobal) {
        el.hintProfileGlobal.title =
    ...
    function updateDetectedVars() {
      ...
      if (el.hintProfileDetected) {
        el.hintProfileDetected.title =
          "Names found in the template. Regular names map to CSV columns or per-line fields; globals are filled once for the whole list.";
      }
    ...
    function openEditor(id) {
      var p = state.profiles.find(function (x) {
        return x.id === id;
      });
      if (!p) return;
      state.editingId = id;
    ...
    function openNewProfile() {
      ...
      MM.saveProfiles(state.profiles);
      openEditor(p.id);
    }
    ...
    el.btnDelete.addEventListener("click", function () {
      ...
      state.profiles = state.profiles.filter(function (x) {
        return x.id !== state.editingId;
      });
      MM.saveProfiles(state.profiles);
      closeEditor();
    ...
    return { renderProfileList: renderProfileList, closeEditor: closeEditor, showEditor: showEditor, openEditor: openEditor };
  }

  MM.initProfilesUi = initProfilesUi;
})();
